...
export const SellerDeleteProduct = (id) => async (dispatch, getState) => {
  try {
    dispatch({ type: PRODUCT_DELETE_REQUEST })
    ...
    await axios.delete(`/api/products/seller/${id}`, config)
...
export const SellerUpdateProduct = (product) => async (dispatch, getState) => {
  try {
    dispatch({ type: PRODUCT_UPDATE_REQUEST })
    ...
    const { data } = await axios.put(
      `/api/products/seller/${product._id}`,
      product,
      config
    )
